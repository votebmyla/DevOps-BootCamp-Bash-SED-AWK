...
awk '/saned/ {sub(/usr\/sbin\/nologin/, "bin/bash")} 1' passwd_new > tmp && mv tmp passwd_new
...
awk -F: -v OFS=: '{print $1,$3,$5,$7}' passwd_new > tmp && mv tmp passwd_new
...
awk -F: -v OFS=: '($2%2)==0 {$NF="/bin/zsh"} 1' passwd_new > tmp && mv tmp passwd_new